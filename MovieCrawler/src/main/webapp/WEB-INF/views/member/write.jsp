		...
		<div class="member_body">
			...
			<div class="member_content">
				<form method="POST" id="frm_mem">
					<div id="fcryan_id" class="in-box">
						<h3>
							<label class="join_label">아이디</label>
						</h3>
						<div class="">
							<input type="text" name="userid" id="inputid" class="join_write"
								placeholder="아이디" value="${one.userid}"> <span
								id="join-id" class="join-class"></span>
						</div>
					</div>
					<div class="in-box">
						<h3>
					...
					<div class="in-box">
						...
						<input type="text" name="name" id="inputname" class="join_write"
							value="${one.name}" placeholder="회원 이름"> <span
							id="join-name" class="join-class"></span>
					</div>
					<div class="in-box">
						<h3>
					...
					<div class="in-box">
						...
						<input type="text" name="phone" id="inputPhone" class="join_write"
							value="${one.phone}" placeholder="전화번호( - 없이 입력해주세요)"> <span
							id="join-tel" class="join-class"></span>
					</div>
					<div class="in-box">
						<h3>
						...
						<input type="text" id="sample6_postcode" name="zipcode"
							value="${one.zipcode}" class="addrbtn" placeholder="우편번호"
							readonly="readonly"> <span id="sample6_extraAddress"></span>
						<input type="button" onclick="sample6_execDaumPostcode()"
							id="addr_btn" value="우편번호 찾기" readonly="readonly"><br>
						<input value="${one.addr1}" type="text" id="sample6_address"
							name="addr1" class="addrbtn" placeholder="주소" readonly="readonly">
						<input type="text" value="${one.addr2}" id="sample6_detailAddress"
							class="join_addr" name="addr2" placeholder="상세주소">
			...
			.ready(
					function() {
						...
						if (id != '') {//회원 수정
							...
							$('#join_title').text("회원 수정").css('text-align',
									'center');
							$('#fcryan_id').remove();
							$('.bz-btn.save').text('수정하기');
						...
						$('#inputpw').keyup(
								function() {
									...
									var checkResult = joinValidate.checkPw(
											memPw, memRpw);
						...
						$('#inputrpw')
								.keyup(
										function() {
											var memPw = $.trim($("#inputpw")
													.val());
											var memRpw = $.trim($(this).val());
											var checkResult = joinValidate
													.checkRpw(memPw, memRpw);
											if (checkResult.code != 0) {//경고 메세지 출력
												$(this)
														.next()
												...
												$(this)
														...
														.css('display', 'block')
														.css('color', '#0000ff');
												if ($('#inputpw').next().text() == '현재 비밀번호랑 일치합니다.') {
													$('#inputrpw').next().text(
															'');
												}
												check_pw = true;
						...
						$('#email_id').blur(
								function() {
									...
									var checkResult = joinValidate.checkEmail(
											email, url);
									if (checkResult.code != 0) {
										$('#selmail').next().text(
												checkResult.desc).css(
						...
						$('#email_url').blur(
								function() {
									...
									var url = $.trim($('#email_url').val());
									var checkResult = joinValidate.checkUrl(
											email, url);
									if (checkResult.code != 0) {
										$('#selmail').next().text(
												checkResult.desc).css(
